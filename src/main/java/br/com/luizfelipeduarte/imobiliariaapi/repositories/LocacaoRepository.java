...
public interface LocacaoRepository extends JpaRepository<Locacao,Long>{
	...
	Optional<Locacao> findByImovel(Imovel imovel);
	List<Locacao> findByInquilino(Pessoa id);
	Page<Locacao> findByInquilino(Pessoa id,Pageable pageable);
	
	@Query(value = "SELECT * FROM locacao l JOIN pessoa p ON l.id_inquilino = p.id " 
		+ " WHERE p.nome LIKE %?1%", nativeQuery = true)
	Page<Locacao> findByNomeInquilino(String nome,Pageable pageable);
	
	@Query(value = "SELECT * FROM locacao l JOIN imovel i on l.id_imovel = i.id "
		+ " JOIN pessoa p ON i.id_proprietario = p.id "
		+ " WHERE p.nome LIKE %?1%", nativeQuery = true)
	Page<Locacao> findByNomeProprietario(String nome,Pageable pageable);
